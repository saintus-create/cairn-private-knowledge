...
describe("ResearchStarterCard", () => {
  it("prioritizes a private document for a document-shaped topic", () => {
    const markup = renderToStaticMarkup(<ResearchStarterCard topic="Can you summarize this PDF report?" onStartProject={vi.fn()} onWebsite={vi.fn()} onDocument={vi.fn()} />);
  ...
  it("prioritizes a website for a broad public topic", () => {
    const markup = renderToStaticMarkup(<ResearchStarterCard topic="What does the California statute require?" onStartProject={vi.fn()} onWebsite={vi.fn()} onDocument={vi.fn()} />);
  ...
  it("wires topic-derived primary and alternate actions to the correct callbacks", () => {
    ...
    const card = ResearchStarterCard({ topic: "Can you summarize this PDF report?", onStartProject, onWebsite, onDocument }) as React.ReactElement<{ children: React.ReactNode }>;
    const sectionChildren = React.Children.toArray(card.props.children);
    const actionGroup = sectionChildren[4] as React.ReactElement<{ children: React.ReactNode }>;
    const actions = React.Children.toArray(actionGroup.props.children) as Array<React.ReactElement<{ onClick?: () => void }>>;
